...
| D15 | 2026-09-14 | **AskHub lists providers and has no partners.** The client confirmed no organisation in the registry is an official partner of the AI Hub; the `is_ai_hub_partner` flag (0019) and the filter on `partners_public` are dropped by 0027, and the wording becomes "provider". Providers are created from the admin portal -- the resource modal, the tracker import and the review queue -- so **`npm run seed` is insert-only for `partners`**: it never updates an existing row, and an admin's edits survive a re-seed structurally rather than by the (already false) observation that nothing in `src/` wrote the table | Settled |
