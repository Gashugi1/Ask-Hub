...
| D11 | **PRD §5.8 governs the public header's right-hand slot: no ADMIN button.** Contact keeps the slot, wearing the prototype's outlined-button treatment exactly | The prototype's public header (reference line 26) carries an outlined ADMIN button linking to the portal. PRD §5.8 states "No login link or admin reference anywhere in public navigation", and the header that was already here documented hitting the same wall. Exposing an admin entry point to every anonymous visitor is a security-posture decision, not a styling one, so the D10 ruling that "the prototype governs" was not stretched to cover it. Put to the client separately; ruled that PRD §5.8 governs. Everything else in lines 3-28 — sticky rail, 1180px width, 68px height, search pill, wordmark treatment, nav and hover states — ports verbatim. |
| D10 | **The prototype governs the type scale, superseding PRD §11.** Section headings become 20px (not 37px), the welcome title 25px (not 49px); `--text-h1`, `--text-h2`, `--text-eyebrow`, `--color-eyebrow` and `--color-eyebrow-on-dark` are superseded | Surfaced after Task 4's baseline capture. This branch derived type tokens from PRD §11's "H2 approximately 37px", which contradicts the prototype's 20px section headings — a 46% difference, and the most visible single change in the port. The eyebrow colours likewise answer a PRD line with no prototype counterpart. Put to the client with measurements; ruled that the prototype governs, consistent with D1 and with the instruction that the app match the prototype exactly. The PRD-derived tokens are not deleted wholesale — they are superseded where the prototype states a value, and remain for anything it does not cover. |
